...
const WorkingApp: React.FC = () => {
  ...
  // 🔒 SICHERHEIT: Portable App immer Demo-only
  const [databaseService] = useState(() => {
    if (isElectron && isProduction) {
      console.log('🔒 Portable App - Using secure demo service (no backend access)');
      return new SecureProductionService() as any;
    } else {
      console.log('🛠️ Browser/Dev mode - Using browser compatible service');
      return new BrowserCompatibleDatabaseService() as any;
    }
  });
  ...
  useEffect(() => {
    ...
    // 🔒 Portable App: Automatisches Demo-Login (kein Backend-Zugriff)
    if (isElectron && isProduction) {
      console.log('🔒 Portable App - Auto-login to demo mode');
    ...
    // Browser/Dev: Session-Validierung für echten Backend (optional)
    const validateExistingSession = async () => {
      const savedToken = localStorage.getItem('auth_token');
      if (savedToken) {
        try {
          const user = await databaseService.validateSession(savedToken);
          if (user) {
            setCurrentUser(user);
  ...
  }, [databaseService, isElectron, isProduction]);
  ...
  // Show auth portal if not authenticated (only in browser/dev mode)
  if (!isAuthenticated) {
    // Portable App sollte automatisch authentifiziert sein
    if (isElectron && isProduction) {
      return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="text-center">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-4"></div>
            <p className="text-gray-600">🔒 Portable App wird initialisiert...</p>
          </div>
        </div>
      );
    }

    // Browser/Dev: Login erforderlich
    return (
      <AuthPortal 
  ...
  const navigationItems = [
    ...
    { id: AppView.DATABASE, label: 'Datenbank', icon: Database },
    { id: AppView.USER_MANUAL, label: 'Handbuch', icon: BookOpen },
    { id: AppView.SETTINGS, label: 'Einstellungen', icon: Settings }
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <div className="w-64 bg-white shadow-lg border-r border-slate-200">
        ...
        {/* Demo Banner für Portable App */}
        {isElectron && isProduction && (
          <div className="mx-4 mt-4 mb-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <div className="flex items-center gap-2">
              <ShieldCheck size={16} className="text-amber-600" />
              <div>
                <p className="text-xs font-semibold text-amber-800">Demo Version</p>
                <p className="text-xs text-amber-700">Keine Backend-Verbindung</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-sm text-slate-900">
                {(currentUser as any)?.full_name || currentUser?.username || 'Demo User'}
              </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                {isProduction ? '🔒 Demo-Modus (Sichere Produktionsversion)' : '🛠️ Development-Modus'}
              </p>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 text-green-600">
                ...
                <span className="text-xs">
                  {isProduction ? 'Sichere Demo-Version' : 'Development'}
                </span>
              </div>
            </div>
          </div>
