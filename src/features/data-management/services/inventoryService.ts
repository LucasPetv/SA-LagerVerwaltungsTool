import { InventoryRow, CalculatedKPIs, ColumnMapping } from '../../../shared/types';
...
export class InventoryService {
   ...
   */
  static calculateKPIs(data: InventoryRow[], mapping: ColumnMapping): CalculatedKPIs[] {
    if (data.length === 0) return [];

    try {
      // Transform raw data to calculated KPIs
      const calculated = data.map((item) => this.applyKPIs(item));
      
      // Apply ABC analysis
      const withABC = this.applyABC(calculated);
      
      console.log(`✅ KPI calculation completed: ${calculated.length} items processed`);
      return withABC;
    } catch (error) {
